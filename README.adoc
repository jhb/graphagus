...
Server mode. Both works fine, here we use the direct mode, without
ZEO. We also use a FileStorage. There are other backends as well.
...
configure the catalogs. The catalogs are optional, but we use them
later on, so lets set them up right now.
...
Sticking to these directories as much as possible is the key to doing very fast queries, because one avoids unpickling of the edges and nodes. 
